...
public class NumberSubProcessor: ILexerSubProcessor
{
    public IEnumerable<Token> MatchTokens(SourceText source, LexerState state)
    {
        ...
        if (TryParseInt(source, state.CurrentPosition, out var intResult, out var intLength))
        {
            results.Add(new Token(Tokens.IntLiteral, intResult, source.Lines.GetLinePosition(state.CurrentPosition), intLength, new List<ILexerStateChangeAction>()));
        }
        
        if (TryParseFloat(source, state.CurrentPosition, out var floatResult, out var floatLength))
        {
            results.Add(new Token(Tokens.FloatLiteral, intResult, source.Lines.GetLinePosition(state.CurrentPosition), floatLength, new List<ILexerStateChangeAction>()));
        }
